...
double ae1[NJ][NI], as1[NJ][NI], aw1[NJ][NI], an1[NJ][NI], ap1[NJ][NI]; // Coefficients

int main()
{
	mkdir("AI");
	double TMAX=7.01;
	int WRITE_INT_CONTOUR=100;
	grid_generation();
	initial_condition();
	boundary_condition();
	WRITE_FILE();
	...
	for (i = 1;i < NCELLI;i++)
	{
		...
		for (i = 1;i < NCELLI;i++)
		{
			...
			aw1[j][i] = fabs((mw * s0[j][i]) );
			as1[j][i] = fabs((ms * s1[j][i]) );
			ap1[j][i] = fabs((me*s2[j][i])+(mn*s3[j][i]));
			an1[j][i] = 0;

			ap1[j][i] =   aw1[j][i] + as1[j][i];
                    ...
                    for (j = 0;j <= NCELLJ;j++)
                {
                   ...
                   {		l=0;
                            for (j = 1;j < NCELLJ;j++)
                        {
                            for(i=1;i < NCELLI;i++)
                            {
                                PHI[j][i] = ((q_gen*DELT)/(rho*cp)) + PHI_OLD[j][i] + (DELT/(VOL[j][i])*rho)*(aw1[j][i]*PHI_INT[j][i-1] +ae1[j][i]*PHI_INT[j][i+1] + as1[j][i]*PHI_INT[j-1][i] - an1[j][i]*PHI_INT[j+1][i] - ap1[j][i]*PHI_INT[j][i]);
	...
	for (j = 1;j < NCELLJ;j++)
	{
		...
		void WRITE_FILE()
		{
			...
			ofstream file;
			stringstream ss;
			ss << "AI/TEMP_PROFILE_TIME_" << NSTEP*DELT << ".dat";
